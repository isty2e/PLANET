...
class VirtualScreening_Dataset(Dataset):
    ...
    def mol_index_from_sdf(self):
        ...
        index_list = [
            index_list[i : i + self.batch_size]
...
def predict(model: PLANET, protein_pdb, ligand_sdf, dataloader):
    ...
    fresidues = model.cal_res_features_helper(
        pocket.res_features, pocket.alpha_coordinates
    )
    ...
    for mol_feature_batch, mol_smiles in dataloader:
        ...
        predicted_affinity = model.screening(
            fresidues_batch, res_scope, mol_feature_batch
        )
        predicted_affinities.append(
            np.array(predicted_affinity.squeeze().detach().cpu().reshape([-1]))
        )
...
def workflow(model, folder_path, out_folder):
    ...
    predicted_active_affinities, active_smiles = predict(
        model, pdb_path, crystal_ligand, active_loader
    )
    predicted_decoy_affinities, decoy_smiles = predict(
        model, pdb_path, crystal_ligand, decoy_loader
    )
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for folder in os.listdir(args.DUDE):
            ...
            predicted_active_affinities, predicted_decoy_affinities = workflow(
                model=PLANET, folder_path=work_folder, out_folder=out_folder
            )
            sorted_array, auc_score = EF_sort(
                predicted_active_affinities, predicted_decoy_affinities
            )
        ...
        [
            {
                ...
                "Active Median": active_median,
                "Decoy Mean": decoy_mean,
                "Decoy Median": decoy_median,
                "Delta Mean": active_mean - decoy_mean,
                "Delta Median": active_mean - decoy_median,
                "EF_0.5%": x,
                "EF_1%": y,
                "EF_2%": z,
                "EF_5%": w,
                "ROC_AUC Score": auc_score,
            }
            for target_name, active_mean, active_median, decoy_mean, decoy_median, x, y, z, w, auc_score in zip(
                target_names,
                active_means,
                active_medians,
                decoy_means,
