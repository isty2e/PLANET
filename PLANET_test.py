...
def test_PLANET(PLANET, test_pickle):
    test_dataset = ProLigDataset(
        test_pickle, batch_size=16, shuffle=False, decoy_flag=False
    )
    ...
    with torch.no_grad():
        for res_feature_batch, mol_feature_batch, targets in test_loader:
            try:
                (
                    predicted_lig_interaction,
                    predicted_interaction,
                    predicted_affinity,
                ) = PLANET(res_feature_batch, mol_feature_batch)
                ligand_interaction, pro_lig_interaction, pK, _, _ = targets

                predicted_lig_interactions.append(
                    np.array(predicted_lig_interaction.squeeze().detach().cpu())
                )
                predicted_interactions.append(
                    np.array(predicted_interaction.squeeze().detach().cpu())
                )
                predicted_affinities.append(
                    np.array(predicted_affinity.squeeze().detach().cpu())
                )
                lig_scopes.extend(mol_feature_batch[4])
                res_scopes.extend(res_feature_batch[2])

                ligand_interactions.append(
                    np.array(ligand_interaction.squeeze().detach().cpu())
                )
                pro_lig_interactions.append(
                    np.array(pro_lig_interaction.squeeze().detach().cpu())
                )
    ...
    RMSE = np.sqrt(
        np.sum(np.square(predicted_affinities - pKs)) / len(predicted_affinities)
    )
    ...
    print(
        "MAE:{:3f}\tRMSE:{:3f}\tPearson:{:3f}\tP_pvalue:{:3f}\tSpearman:{:3f}\tS_pvalue:{:3f}"
        .format(MAE, RMSE, P_correlation, P_pvalue, S_correlation, S_pvalue)
    )
