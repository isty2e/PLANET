...
class PlanetEstimator:
    ...
    def set_pocket_from_coordinate(
        self, protein_pdb, centeriod_x, centeriod_y, centeriod_z
    ):
        try:
...
class VS_SMI_Dataset(Dataset):
    ...
    def tensorize(self, idx):
        ...
        mol_batch_contents = [
            (sanitize_mol(smi), smi, name)
            for (smi, name) in mol_batch_contents
            if Chem.MolFromSmiles(smi, sanitize=True) is not None
        ]
        mol_feature_batch = mol_batch_to_graph(
            [content[0] for content in mol_batch_contents]
        )
...
def workflow(
    ...
    if ligand_sdf is not None:
        ...
        centeriod_x is not None and centeriod_y is not None and centeriod_z is not None
    ):
        estimator.set_pocket_from_coordinate(
            protein_pdb, centeriod_x, centeriod_y, centeriod_z
        )
    else:
        sys.exit()
    ...
    if suffix == "smi":
        ...
        raise NotImplementedError(
            "mol file input formats besides smi and sdf are not supported"
        )
    ...
    with torch.no_grad():
        for mol_feature_batch, smi_batch, mol_name in dataloader:
            ...
            fresidues_batch, res_scope = estimator.model.cal_res_features(
                estimator.res_features, batch_size
            )
            predicted_affinity = estimator.model.screening(
...
if __name__ == "__main__":
    ...
    protein_pdb, mol_file, ligand_sdf, centeriod_x, centeriod_y, centeriod_z = (
        args.protein,
        args.mol_file,
        args.ligand,
        args.center_x,
