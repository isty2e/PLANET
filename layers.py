...
class ProteinGAT(nn.Module):
    def __init__(
        self, feature_dims, nheads, key_dims, value_dims, update_iters, device
    ):
        super().__init__()
        self.nheads = nheads
        ...
        self.linear_out = nn.Sequential(
            nn.Linear(value_dims, feature_dims), nn.LeakyReLU(0.1)
        )
    ...
    def split(self, q, k, v):
        q = torch.reshape(
            q, shape=[self.node_size, self.nheads, self.key_dims]
        ).permute(
            [1, 0, 2]
        )  # [h,n,k]
        k = torch.reshape(
            k, shape=[self.node_size, self.nheads, self.key_dims]
        ).permute(
            [1, 2, 0]
        )  # [h,k,n]
        v = torch.reshape(
            v, shape=[self.node_size, self.nheads, self.value_dims]
        ).permute(
            [1, 0, 2]
        )  # [h,n,v]
        return q, k, v
    ...
    def forward(self, fresidues, distance_weight, res_mask):
        ...
        fresidues = fresidues + torch.sum(
            torch.unsqueeze(fresidues, 0) * distance_weight, dim=1
        )
...
class LigandGAT(nn.Module):
    ...
    def split(self, q, k, v):
        q = torch.reshape(
            q, shape=[self.bonds_size, 1, self.nheads, self.key_dims]
        ).permute(
            [0, 2, 1, 3]
        )  # [b,1,h,k] -- #[b,h,1,k]
        k = torch.reshape(
    ...
    def forward(self, fatoms, fbonds, agraph, bgraph, lig_scope):
        ...
        for _ in range(self.update_iters - 1):
            nei_message = index_select_ND(
                message, 0, bgraph
            )  # [fbonds,nei,feature_dim]
...
class ProLig(nn.Module):
    def __init__(self, feature_dims, nheads, update_iters, device):
        ...
        self.feature_dims = feature_dims
        self.device = device
        self.prolig_attention = ProteinLigandAttention(
            feature_dims, nheads, update_iters, device
        )
    ...
    def forward(self, fresidues, fatoms, res_scope, lig_scope):
        fatoms, fresidues = self.prolig_attention(
            fatoms, fresidues, lig_scope, res_scope
        )
        ...
        for (start_res, res_count), (start_atom, atom_count) in zip(
            res_scope, lig_scope
        ):
            ...
            predicted_interactions.append(
                torch.reshape(predicted_interaction, shape=[-1])
            )
    ...
    def forward(self, fresidues, coordinates, res_scope):
        ...
        for _ in range(self.update_iters):
            ...
            for start_res, res_count in res_scope:
                ...
                coor_diff = coordinates_batch.unsqueeze(
                    1
                ) - coordinates_batch.unsqueeze(0)
                m_input = torch.cat(
                    [
                ...
                fresidues_batch = (
                    self.linear_h(torch.cat([fresidues_batch, m_pre], dim=-1))
                    + fresidues_batch
                )
