...
class ProLigDataset(Dataset):
    ...
    def check(
        self, data
    ):  # check each batch has at least one record with pK value (that is from original PDBbind)
        for batch in data:
    ...
    def tensorize(self, idx):
        ...
        return (
            res_feature_batch,
            mol_feature_batch,
            (mol_intearctions, pro_lig_interactions, pKs, pK_flags, complex_labels),
        )
...
if __name__ == "__main__":
    ...
    for csv, _type_ in zip(
        [train_csv, train_affinity_csv, valid_csv, core_csv],
        ["train", "train_affinity", "valid", "core"],
    ):
        ...
        for pdb_code, record_type, pK in zip(pdb_codes, record_types, pKs):
            ...
            if os.path.exists(
                os.path.join(
                    pdb_dir, sub_folder, pdb_code, "{}_pocket.pkl".format(pdb_code)
                )
            ):
                with open(
                    os.path.join(
