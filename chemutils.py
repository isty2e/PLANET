...
def near_pocket(pdb_line, centeroid):
    ...
    distance = np.sqrt(
        np.sum(np.square(np.array([x, y, z], dtype=np.float32) - centeroid))
    )
    if distance > 12.0:
        return False
    else:
...
def mass_center_from_pdb(pdb_lines):
    pdb_lines = [
        ...
        if line[76:78].strip().capitalize() in PROTEIN_ELEMENTS
    ]
    atoms_mass = np.reshape(
        np.array(
            [
                periodic_table.GetAtomicWeight(line[76:78].strip().capitalize())
                for line in pdb_lines
            ]
        ),
        [-1, 1],
    ...
    mass_center = np.sum(coordinates * atoms_mass, axis=0, keepdims=True) / np.sum(
        atoms_mass
    )
...
class ProteinPocket:
    def __init__(
        ...
        if ligand_sdf is not None:
            ...
            centeriod_x is not None
            and centeriod_y is not None
            and centeriod_z is not None
        ):
            centeriod = np.array(
                [centeriod_x, centeriod_y, centeriod_z], dtype=np.float32
            )
        ...
        self.alpha_coordinates = torch.from_numpy(
            np.concatenate(
                [residue.get_alpha_position() for residue in self.pocket_residues],
                axis=0,
            )
        )
    ...
    def determine_pocket_residues(self, protein_pdb, centeroid):
        ...
        pocket_atoms_content = [
            line for line in pdb_content if near_pocket(line, centeroid)
        ]
...
class ComplexPocket:
    def __init__(self, protein_pdb, ligand_sdf, pK=0, decoy_sdf=None):
        ...
        if decoy_sdf is not None and os.path.exists(decoy_sdf):
            self.decoys = [
                Mol(mol)
                for mol in Chem.SDMolSupplier(decoy_sdf, removeHs=False)
                if mol is not None
            ]
        else:
            self.decoys = []
        ...
        pocket_atoms_content = [
            line
            for line in pdb_content
            if near_pocket(line, self.ligand.compute_centeroid())
        ]
    ...
    def get_interaction_label(self):
        pro_lig_interaction = np.zeros(
            shape=[self.ligand.mol.GetNumAtoms(), len(self.pocket_residues)],
            dtype=np.float32,
        )
        ...
        for atom in self.ligand.mol.GetAtoms():
            ...
            for residue_index in range(len(self.pocket_residues)):
                for res_atom_coordinate in self.pocket_residues[
                    residue_index
                ].coordinates:
                    if (
                        np.sqrt(
                            np.sum(np.square(res_atom_coordinate - atom_coordinate))
                        )
                        > 4.0
                    ):
                        continue
    ...
    def get_distance_matrix(self):
        mass_coordinates = np.array(
            [residue.get_mass_center() for residue in self.pocket_residues],
            dtype=np.float32,
        )
        ...
        mass_matrix = np.sqrt(
            np.sum(
                np.square(
                    mass_coordinates.reshape([-1, 1, 3])
                    - mass_coordinates.reshape([1, -1, 3])
                ),
                axis=-1,
                keepdims=True,
        ...
        alpha_matrix = np.sqrt(
            np.sum(
                np.square(
                    alpha_coordinates.reshape([-1, 1, 3])
                    - alpha_coordinates.reshape([1, -1, 3])
                ),
                axis=-1,
                keepdims=True,
    ...
    def process_alternate(self, contents):
        ...
        if len(alternate_flag) > 0:
            contents = [
                ...
                if line[16] == alternate_flag[0] or line[16] == " "
            ]
        return contents
...
class Mol:
    ...
    def get_interaction_label(self):
        ...
        atom_distances = np.sqrt(
            np.sum(
                np.square(
                    atom_coordinates.reshape(-1, 1, 3)
                    - atom_coordinates.reshape(1, -1, 3)
                ),
                axis=-1,
            )
...
def random_ligand_decoy(protein_pockets, decoy_flag):
    if decoy_flag:
        ...
        while check:
            ...
            for pocket in protein_pockets:
                if pocket.pK != 0:
                    complex_labels.append(
                        int(np.random.choice([0, 1], size=1, p=[0.5, 0.5]))
                    )
                else:
                    complex_labels.append(
                        int(np.random.choice([0, 1], size=1, p=[0.5, 0.5]))
    ...
    for i, complex_label in enumerate(complex_labels):
        if complex_label == 1:
            mol_batch.append(
                protein_pockets[i].ligand
            )  # ligand (self-designed class Mol)
...
def tensorize_all(protein_pockets, decoy_flag=True):
    pKs, pK_flags, complex_labels, mol_batch = random_ligand_decoy(
        protein_pockets, decoy_flag
    )
    ...
    for i, label in enumerate(complex_labels):
        if label == 1:
            pro_lig_interactions.append(
                protein_pockets[i].pro_lig_interaction.reshape([-1])
            )
        else:
            pro_lig_interactions.append(
                np.zeros(
                    [mol_batch[i].atom_count, protein_pockets[i].res_count]
                ).reshape([-1])
            )
    pro_lig_interactions = torch.from_numpy(
        np.concatenate(pro_lig_interactions).astype(np.float32)
    )
    return (
        res_feature_batch,
        mol_feature_batch,
...
def tensorize_protein_pocket(protein_pockets):
    ...
    fresidues = torch.from_numpy(
        np.reshape(np.array(fresidues, dtype=np.float32), [-1, 20])
    )
...
def tensorize_ligand(mol_batch: List[Mol]):
    (fatoms, fbonds, agraph, bgraph, lig_scope) = mol_batch_to_graph(
        [mol.mol for mol in mol_batch]
    )
    mol_intearctions = [mol.get_interaction_label().reshape([-1]) for mol in mol_batch]
    mol_intearctions = torch.from_numpy(
        np.concatenate(mol_intearctions).astype(np.float32)
    )
...
def sanitize_mol(smi):
    return Chem.AddHs(
        Chem.MolFromSmiles(
            Chem.MolToSmiles(
                Chem.MolFromSmiles(smi, sanitize=True), isomericSmiles=False
            )
        )
    )
