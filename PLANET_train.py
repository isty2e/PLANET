...
if __name__ == "__main__":
    ...
    valid_dataset = ProLigDataset(
        args.valid, args.batch_size, shuffle=False, decoy_flag=False
    )
    ...
    test_dataset = ProLigDataset(
        args.test, args.batch_size, shuffle=False, decoy_flag=False
    )
    ...
    (
        ...
        args.key_dims,
        args.value_dims,
        args.pro_update_inters,
        args.lig_update_iters,
        args.pro_lig_update_iters,
    )
    PLANET = PLANET(
        feature_dims,
        nheads,
    ...
    print(
        "Model #Params: {:d}K".format(
            sum([x.nelement() for x in PLANET.parameters()]) // 1000
        )
    )
    print(PLANET)
    optimizer = optim.Adam(
        filter(lambda p: p.requires_grad, PLANET.parameters()), lr=args.lr
    )
    ...
    for epoch in range(1, 1 + args.epoch):
        if not affinity_all:
            train_dataset = ProLigDataset(
                args.train, args.batch_size, shuffle=True, decoy_flag=True
            )
            ...
            train_dataset = ProLigDataset(
                args.train_affinity, args.batch_size, shuffle=True, decoy_flag=False
            )
            train_loader = DataLoader(
                train_dataset,
                batch_size=1,
        ...
        for res_feature_batch, mol_feature_batch, targets in train_loader:
            ...
            lig_interaction_loss, pro_lig_interaction_loss, affinity_loss = (
                PLANET.compute_loss(
                    predictions, targets, res_feature_batch, mol_feature_batch
                )
            )
            total_loss = (
                lig_interaction_loss + pro_lig_interaction_loss + beta * affinity_loss
            )
            total_loss.backward()
            nn.utils.clip_grad_norm_(
                filter(lambda p: p.requires_grad, PLANET.parameters()), args.clip_norm
            )
            optimizer.step()

            lig_interaction_acc, pro_lig_interaction_acc, affinity_mae = (
                PLANET.compute_metrics(predictions, targets)
            )

            meters = meters + np.array(
                [
                    lig_interaction_loss.item(),
                    lig_interaction_acc.item(),
                    pro_lig_interaction_loss.item(),
                    pro_lig_interaction_acc.item(),
                    affinity_loss.item(),
                    affinity_mae.item(),
                ]
            )
            ...
            if total_step % args.print_iter == 0:
                ...
                print(
                    "[{}]\tLig_L:{:.3f}\tLig_ACC:{:.3f}\tProLig_L:{:.3f}\tProLig_ACC:{:.3f}\tAffinity_L:{:.3f}\tMAE:{:.3f}"
                    .format(
                        total_step,
                        meters[0],
                        meters[1],
            ...
            if total_step > 0 and total_step % args.save_iter == 0:
                ...
                with torch.no_grad():
                    ...
                    for res_batch, mol_batch, targets in valid_loader:
                        try:
                            ...
                            (
                                lig_interaction_loss,
                                pro_lig_interaction_loss,
                                affinity_loss,
                            ) = PLANET.compute_loss(
                            ...
                            (
                                lig_interaction_acc,
                                pro_lig_interaction_acc,
                                affinity_mae,
                            ) = PLANET.compute_metrics(predictions, targets)
                            valid_batch_count += 1
                            valid_meters = valid_meters + np.array(
                                [
                                    lig_interaction_loss.item(),
                                    lig_interaction_acc.item(),
                                    pro_lig_interaction_loss.item(),
                    ...
                    print(
                        "[Valid_{}]\tLig_L:{:.3f}\tLig_ACC:{:.3f}\tProLig_L:{:.3f}\tProLig_ACC:{:.3f}\tAffinity_L:{:.3f}\tMAE:{:.3f}"
                        .format(
                            total_step,
                            valid_meters[0],
                            valid_meters[1],
                ...
                torch.save(
                    PLANET.state_dict(),
                    args.save_dir + "/PLANET.iter-" + str(total_step),
                )
                ...
                with torch.no_grad():
                    ...
                    for res_batch, mol_batch, targets in test_loader:
                        try:
                            ...
                            test_batch_count += 1
                            test_meters = test_meters + np.array(
                                [
                                    lig_interaction_loss.item(),
                                    lig_interaction_acc.item(),
                                    pro_lig_interaction_loss.item(),
                    ...
                    print(
                        "[Test_{}]\tLig_L:{:.3f}\tLig_ACC:{:.3f}\tProLig_L:{:.3f}\tProLig_ACC:{:.3f}\tAffinity_L:{:.3f}\tMAE:{:.3f}"
                        .format(
                            total_step,
                            test_meters[0],
                            test_meters[1],
